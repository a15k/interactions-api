...
RSpec.describe ApiV0Helpers, api: :v0 do
  ...
  context "#prep_request_args" do
    ...
    it "adds blank headers if none set" do
      expect(prep_request_args(['blah'])[1]).to eq ({headers: {}})
    end

    it "adds headers if some set" do
      headers["foo"] = "bar"
      expect(prep_request_args(['blah'])[1]).to eq ({headers: {"foo" => "bar"}})
    end

    it "does not override explicitly set headers" do
      headers["foo"] = "bar"
      expect(prep_request_args(['blah', {headers: {"foo" => "keepthis"}}])[1]).to eq (
        {headers: {"foo" => "keepthis"}}
      )
    end
  end
